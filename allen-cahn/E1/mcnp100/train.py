...
def p_matrix(config):
    ...
    for i in range(1, size_x):
        if x[i] <= 2/3:
            d = x[i]
            p[i, :] = (1/(math.sqrt(2*torch.pi) * sigma) * np.exp(-((-x+d)**2)/(2 * sigma**2))) * dx - (1/(math.sqrt(2*torch.pi) * sigma) * np.exp(-((-x+d-2*d)**2)/(2 * sigma**2))) * dx        
        if x[i] > 2/3:
...
def train(config, net):
    ...
    val_data = torch.load(config.data_path + 'data_test').to(device).float()
    ...
    for step in range(config.num_iterations+1):
        u_0 = generate_u0(batch_size, N, 1024, device)
        u_0 = (u_0[:, 15:-1:16] + u_0[:, 16::16])/2
        u_0 = torch.concat([torch.zeros(batch_size, 1, 1, device=device), u_0, torch.zeros(batch_size, 1, 1, device=device)], dim=1)
        net.train()
